...
# Setup directories
RESULTS_DIR = Path(__file__).parent / "results" / "main3"
...
def run_vae_experiment(dataset_name: str, n_epochs: int = 50) -> List[Dict[str, Any]]:
    ...
    VAE is adapted for classification using reconstruction error with threshold.
    ...
    print(f"Dataset info: features={n_features}, seq_length={seq_length}, classes={n_classes}")
    print(f"Device: {device}")
    print("Note: VAE adapted for classification using reconstruction error")
    ...
    for idx, combo in enumerate(combinations, 1):
        ...
        # Custom training and evaluation for VAE-based classification
        ...
        for epoch in range(n_epochs):
            ...
            for batch in train_loader:
                ...
                loss, recon_loss, kl_loss = model.loss_function(x, reconstruction, mu, logvar)
                loss.backward()
                optimizer.step()
            ...
            epoch_history.append({'epoch': epoch, 'train_loss': avg_loss})
        ...
        # Evaluation - use reconstruction error for classification
        ...
        # Average scores across sequence length
        if all_scores.ndim > 1:
            all_scores = all_scores.mean(axis=1)

        # Use median as threshold for binary/multi-class classification
        threshold = np.median(all_scores)

        # For multi-class: cluster anomaly scores into n_classes groups
        # Simple approach: use quantiles
        quantiles = np.linspace(0, 100, n_classes + 1)
        thresholds = [np.percentile(all_scores, q) for q in quantiles]

        predictions = np.digitize(all_scores, thresholds[1:-1])
        ...
        results.append(result_dict)
        print(f"Accuracy: {accuracy:.4f}, F1: {f1_macro:.4f}, Threshold: {threshold:.4f}")
